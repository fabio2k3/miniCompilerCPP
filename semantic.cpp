...
public:
	...
	std::string getType(const std::string& name) const{
		...
		if(it == symbols.end()){
			throw std::runtime_error("Error sem�ntico: variable '" + name + "' no definida");
		}
		return it -> second;
	}
	...
	std::string analyzeExpression(const ASTNode* node){
		...
		if(auto* binOp = dynamic_cast<const BinaryOpNode*>(node)){
			...
			if(leftType != "number" || rightType != "number")
				throw std::runtime_error("Error sem�ntico: operaci�n '" + binOp ->op + "' requiere operandos num�ricos");

			return "number";
		}
		throw std::runtime_error("Error sem�ntico: nodo de expresi�n desconocido");
	}
